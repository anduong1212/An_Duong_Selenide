...
public class LeapFrogTabProcessingTask implements Callable<TabResult> {
    ...
    private final ProductsData EXPECTED_DATA_LOADER = new ProductsData();

    public LeapFrogTabProcessingTask(TabConfig tabConfig) {
        this.TAB_CONFIG = tabConfig;
        this.logger = Logger.getLogger(LeapFrogTabProcessingTask.class.getName());
    }
    @Override
    public TabResult call() throws Exception {
        ...
        for(int pageNumber = TAB_CONFIG.startPage(); pageNumber <= TAB_CONFIG.endPage(); pageNumber++) {
            ...
            try {
                ...
                List<Product> expectedProductList = EXPECTED_DATA_LOADER.getProducts();
